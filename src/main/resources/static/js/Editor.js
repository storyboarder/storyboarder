define(["jsPDF", "./CanvasState", "./tools/Toolset", "./tools/SnapUtil"], function(jsPDF, canvasState, toolset, snapUtil) {
			...
			var actions = {
				...
				"SaveLargePage": function(params) {
					...
					var currJsonObj = getCurrentPageJSON();
					var currJsonString = currJsonObj.json;

					var len = currJsonString.length;

					var firstHalf = currJsonString.substring(0, len / 2);
					var secondHalf = currJsonString.substring(len / 2, len);

					currJsonObj.json = firstHalf;

					$.post("/pages/saveLargeFirst", currJsonObj, function(response) {
								console.log("saved first half:", currJsonObj, "response:", JSON.parse(response));
								currJsonObj.json = secondHalf;
								$.post("/pages/saveLargeSecond", currJsonObj, function(response) {
										console.log("saved second half:", currJsonObj, "response:", JSON.parse(response));
									});
								});
